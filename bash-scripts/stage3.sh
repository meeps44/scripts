...
pt_run()
{
    ...
    echo "Timestamp: $(date -u +'%Y-%m-%dT%H:%M:%SZ')" > $l_FILEPATH$l_FILENAME # Print the current date in UTC and ISO 8601 format
    echo "Metadata: $l_FILEPATH$l_FILENAME $HOSTNAME ${l_DESTINATION_PORT} ${HOST_IP} ${l_FLOW_LABEL}"
