...
# Also generates a hitlist based on the destinations where the flowlabel
            ...
            for file in os.listdir(args.directory):
                if (os.path.isfile(os.path.join(args.directory, file))):
                    ...
                    with open(os.path.join(args.directory, file), 'r') as file:
                        ...
                        data = json.load(file)
                        for item in data:
                            vantage_point_list.append(item['source'])
                            destination_ip = item['destination']
                            source_flow_label = int(item['flow_label'])
                            source_flow_label_list.append(source_flow_label)
                            tcp_port = item['outgoing_tcp_port']
                            flow_label_changed = False
                            hop_list = [] 

                            for key, value in item['hops'].items():
                                try:
                                    if (item['hops'][key]['returned_flow_label'] != source_flow_label):
                                        flow_label_changed = True
                                        changed_counter = changed_counter + 1
                                        hop_number = key
                                        changed_location.append(hop_number)
                                        hop_ip = item['hops'][key]['ipv6_address'] 
                                        hop_flow_label = item['hops'][key]['returned_flow_label'] 
