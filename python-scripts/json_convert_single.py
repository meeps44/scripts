...
def parse(directory, filename, tcp_port, source_ip, flow_label):
    try:
        if (os.path.isfile(os.path.join(directory, filename))):
            with open(os.path.join(directory, filename), "r") as f:
                file_data = f.read()
                convert(tcp_port, source_ip, flow_label, file_data)
...
def main():
    ...
    json_data = parse(args.directory, args.file, args.tcp_port, args.source_ip, args.flow_label)
